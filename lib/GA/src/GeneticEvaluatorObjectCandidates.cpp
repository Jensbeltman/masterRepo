...
GeneticEvaluatorOC::GeneticEvaluatorOC(std::vector<T4> &poses, PointCloudT::Ptr &pc, PointCloudT::Ptr &pcm,
                                       NormalCloudT::Ptr &ncm, std::shared_ptr<cv::viz::Mesh> &meshptr, T4 &camera_pose,
                                       double inlier_threshold) : pc(pc), pcm(pcm), object_candidates(poses),
                                                                  inlier_threshold(inlier_threshold) {
    Chronometer chronometer;
    chronometer.tic();

    // KdTree of cloud data
    pcl::KdTreeFLANN<PointT>::Ptr kdt(new pcl::KdTreeFLANN<PointT>());
    kdtree = kdt;
    ...
    std::cout << "Kdtree init elapsed time: " << chronometer.toc() << "s\n";
    chronometer.tic();
...
//    }
    ...
    std::cout << "DatasetObject Collision test: " << chronometer.toc() << "s\n";
